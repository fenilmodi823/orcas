...
export type CatalogOrigin = 'live' | 'cached' | 'unavailable';
 ...
 * persist it; on any failure, fall back to whatever was last persisted to
 * IndexedDB rather than showing nothing. Matches Rules.md's error-handling
 * table: "Backend unreachable -> scene runs from the static snapshot,
 * visible 'data may be stale' pill, never blank." `origin` tells the
 * caller which path served the current snapshot, so the UI can be honest
 * about it — it's fetch-session provenance, not part of the snapshot's
 * own data, so it lives in this hook's state rather than in
 * CatalogSnapshot itself.
 */
