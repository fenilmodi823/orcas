...
If any case fails: read the printed actual vs. expected values, compute the real max diff, and loosen the `toBeCloseTo` precision argument to comfortably clear it. Do not loosen past what's actually observed — the point of this test is knowing the real number, not making it green. Record the final, empirically-confirmed tolerance in a comment above the assertions.

Expected once tuned: PASS, all cases. (Confirmed during implementation: precision 4 failed on every case; a diagnostic pass over all 51 cases without early-exit found the true max diff — 5.63m position / 6.3mm/s velocity, satnum 88888 at tsince=1440min — and precision 1 position / precision 4 velocity, as written above, clears it with 8-9x margin.)
...
describe('buildSegment', () => {
  it('produces endpoints consistent with two direct propagate() calls', () => {
    ...
    // p0/v0 are rotated to J2000 by buildSegment — compare against a
    // reference rotated the same way (T is ~26 years past J2000 for a
    // 2026 epoch, so the rotation is not negligible; see the residual
    // test below for why a raw-TEME comparison here would be wrong).
    const matrix = temeToJ2000Matrix(t0);
    const direct0J2000 = applyMat3(matrix, propagate(satrec, t0, record.NORAD_CAT_ID).positionEciKm);
...
describe('interpolation residual against direct SGP4', () => {
  it.each(Object.entries(REGIME_FIXTURES))(
    ...
    (regime, record) => {
      ...
      // before storing it (for a 2026 epoch that rotation is ~250-270km
      // at GEO radius — T is ~26 years past J2000, not near it), so the
      // "direct" reference must be rotated too, or the comparison is
      // TEME-vs-J2000, not an interpolation-accuracy check.
      function directJ2000(at: Date) {
        const state = propagate(satrec, at, record.NORAD_CAT_ID);
      ...
      // (Date construction truncates a fractional ms), so an
      // independently-recomputed sample point can land 1ms past the
      // chain's actual coverage at the very last sample.
      const chainEndMs = chain[chain.length - 1].t1Ms;

      const sampleCount = 50;
      for (let i = 0; i <= sampleCount; i++) {
        // Rounded to an integer ms *before* use, for both curves, and
        // clamped to the chain's real range: new Date(atMs) truncates a
        // fractional ms internally, so an un-rounded atMs used for
        // propagate() at one instant and sampleChain() (which takes the
        // raw float) at a different, sub-ms-shifted instant produces a
        // spurious few-metre "residual" at LEO orbital speed (~7.66
        // m/ms) that is a measurement artifact, not interpolation
        // error — and the very last sample, unclamped, falls 1ms past
        // the chain's real end for the same truncation-vs-rounding
        // reason, triggering sampleChain's out-of-range fallback.
        const atMs = Math.min(Math.round(EPOCH_MS + (periodMs * i) / sampleCount), chainEndMs);
        const directPosition = directJ2000(new Date(atMs));
...
If the residual test fails, don't assume `chooseStepSeconds` or the Hermite math is wrong — the two most likely causes, both found and fixed during this plan's own implementation, are measurement bugs in the test, not the production code:

1. **TEME-vs-J2000 frame mismatch.** `buildSegment` rotates to approximate-J2000 before storing endpoints; comparing against a raw (un-rotated) `propagate()` call gives a ~250-270km "residual" for a 2026 epoch (T is ~26 years past J2000, not near it — the rotation is not negligible). Fix: rotate the "direct" reference the same way (`directJ2000`, as written above).
2. **Sub-millisecond time misalignment.** `new Date(atMs)` truncates a fractional millisecond internally, but `sampleChain`/`sampleSegment` take the raw float `atMs` with no `Date` involved. If the "direct" and "interpolated" comparisons don't round to the same integer millisecond first, they can end up evaluated ~1ms apart — at LEO orbital speed (~7.66 m/ms) that alone produces a multi-metre "residual" that has nothing to do with interpolation quality. The very last sample point is the worst offender: `buildSegmentChain`'s real last segment ends at `Math.trunc(endMs)`, so an independently-recomputed `Math.round(periodMs)` sample can land 1ms past the chain's actual coverage, triggering `sampleChain`'s out-of-range fallback. Fix: round `atMs` to an integer ms before use, and clamp to `chain[chain.length-1].t1Ms` (both written above).

If a genuine mid-segment residual remains after both fixes (confirm it's mid-segment, not at a boundary, by checking which segment contains the worst sample and its `s` value — not near 0 or 1), that's real interpolation error. Molniya in particular is expected to exceed the brief's simplified circular-motion error formula somewhat, since `chooseStepSeconds` only tightens `h` for perigee's peak rate, not a full per-true-anomaly bound — loosen only that regime's threshold, with the measured number and reasoning in a comment (as written above), never all regimes uniformly.

Expected: PASS, all tests across all four describe blocks. (Confirmed during implementation: both bugs above were hit and fixed in this order; final measured worst residuals were LEO/MEO/GEO comfortably under 1m and Molniya ~2.57m, informing the 5m threshold above.)
